...
async def start_handler(message: Message, state: FSMContext):
    ...
    elif telegram_id in specialists_telegramm_ids:  # Проверяем специалист/посетитель
...
async def admin_handler(message: Message, state: FSMContext):
    ...
    if message.text == '/admin':
            ...
            await message.answer(text="Команды админа",  # Клавиатура дней
                                 reply_markup=admin_keyboard(conn))
    await state.set_state(AdminStates.L1)

@dp.callback_query(AdminStates.L1)
async def spec_list_handler(callback_query: CallbackQuery, state: FSMContext):
    """ """
    # telegram_id = message.chat.username  # Определение телеграм ид
    ...
    if callback_query.data == '/cust_list':
